...
static int register_hook(lua_State *L, ltf_hook_fn fn) {
...
int l_module_hooks_register_test_run_started(lua_State *L) {
    return register_hook(L, LTF_HOOK_FN_TEST_RUN_STARTED);
}

int l_module_hooks_register_test_started(lua_State *L) {
    return register_hook(L, LTF_HOOK_FN_TEST_STARTED);
}

int l_module_hooks_register_test_finished(lua_State *L) {
    return register_hook(L, LTF_HOOK_FN_TEST_FINISHED);
}

int l_module_hooks_register_test_run_finished(lua_State *L) {
    return register_hook(L, LTF_HOOK_FN_TEST_RUN_FINISHED);
}
...
int l_module_hooks_register_module(lua_State *L) {
    LOG("Registering ltf-hooks module...");
    ...
    LOG("Successfully registere ltf-hooks module.");
    return 1;
}
