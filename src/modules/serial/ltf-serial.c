...
static inline l_module_serial_t *check_port(lua_State *L, int idx) {
    return luaL_checkudata(L, idx, "ltf-serial");
}

int l_module_serial_get_port_by_name(lua_State *L) {
    LOG("Invoked ltf-serial get_port...");
    int s = selfshift(L);
    ...
    luaL_getmetatable(L, "ltf-serial");
    lua_setmetatable(L, -2);

    LOG("Successfully finished ltf-serial get_port.");
    return 1;
}
...
int l_module_serial_get_port_info(lua_State *L) {
    ...
    LOG("Invoked ltf-serial get_port_info...");
    ...
    LOG("Successfully finished ltf-serial get_port_info.");
...
int l_module_serial_list_ports(lua_State *L) {
    ...
    LOG("Invoked ltf-serial list_ports...");
    ...
    LOG("Successfully finished ltf-serial list_ports");
    return 1;
}
...
int l_module_serial_open(lua_State *L) {
    ...
    LOG("Successfully finished ltf-serial open.");
    return 0;
}

int l_module_serial_set_baudrate(lua_State *L) {
    LOG("Invoked ltf-serial set_baudrate...");
    int s = selfshift(L);
    l_module_serial_t *u = check_port(L, s);
    ...
    LOG("Successfully finished ltf-serial set_baudrate.");
    return 0;
}

int l_module_serial_set_bits(lua_State *L) {
    LOG("Invoked ltf-serial set_bits...");
    int s = selfshift(L);
    l_module_serial_t *u = check_port(L, s);
    ...
    LOG("Successfully finished ltf-serial set_bits.");
    return 0;
}
...
int l_module_serial_set_parity(lua_State *L) {
    LOG("Invoked ltf-serial set_parity...");
    int s = selfshift(L);
    l_module_serial_t *u = check_port(L, s);
    ...
    LOG("Successfully finished ltf-serial set_parity.");
    return 0;
}

int l_module_serial_set_stopbits(lua_State *L) {
    LOG("Invoked ltf-serial set_stopbits...");
    int s = selfshift(L);
    l_module_serial_t *u = check_port(L, s);
    ...
    LOG("Successfully finished ltf-serial set_stopbits.");
    return 0;
}
...
int l_module_serial_set_rts(lua_State *L) {
    LOG("Invoked ltf-serial set_rts...");
    int s = selfshift(L);
    l_module_serial_t *u = check_port(L, s);
    ...
    LOG("Successfully finished ltf-serial set_rts.");
    return 0;
}
...
int l_module_serial_set_cts(lua_State *L) {
    LOG("Invoked ltf-serial set_cts...");
    int s = selfshift(L);
    l_module_serial_t *u = check_port(L, s);
    ...
    LOG("Successfully finished ltf-serial set_cts.");
    return 0;
}
...
int l_module_serial_set_dtr(lua_State *L) {
    LOG("Invoked ltf-serial set_dtr...");
    int s = selfshift(L);
    l_module_serial_t *u = check_port(L, s);
    ...
    LOG("Successfully finished ltf-serial set_dtr.");
    return 0;
}
...
int l_module_serial_set_dsr(lua_State *L) {
    LOG("Invoked ltf-serial set_dsr...");
    int s = selfshift(L);
    l_module_serial_t *u = check_port(L, s);
    ...
    LOG("Successfully finished ltf-serial set_dsr.");
    return 0;
}
...
int l_module_serial_set_xon_xoff(lua_State *L) {
    LOG("Invoked ltf-serial set_xon_xoff...");
    int s = selfshift(L);
    l_module_serial_t *u = check_port(L, s);
    ...
    LOG("Successfully finished ltf-serial set_xon_xoff.");
    return 0;
}
...
int l_module_serial_set_flowcontrol(lua_State *L) {
    LOG("Invoked ltf-serial set_flowcontrol...");
    int s = selfshift(L);
    l_module_serial_t *u = check_port(L, s);
    ...
    LOG("Successfully finished ltf-serial set_flowcontrol.");
    return 0;
}

/*----------- reading ------------------------------------------------*/
static inline int read_helper(lua_State *L, int blocking) {
    LOG("Invoked ltf-serial read. Blocking: %d", blocking);
    int s = selfshift(L);
    l_module_serial_t *u = check_port(L, s);
    ...
    LOG("Successfully finished ltf-serial read.");
    return 1;
}
...
static inline int write_helper(lua_State *L, int blocking) {
    LOG("Invoked ltf-serial write. Blocking: %d", blocking);
    int s = selfshift(L);
    l_module_serial_t *u = check_port(L, s);
    ...
    LOG("Successfully finished ltf-serial write.");
    return 1;
}
...
static inline int status_helper(lua_State *L, int direction) {
    LOG("Invoked ltf-serial get_waiting, direction: %d...", direction);
    int s = selfshift(L);
    l_module_serial_t *u = check_port(L, s);
    enum sp_return r =
    ...
    LOG("Successfully finished ltf-serial get_waiting.");
...
int l_module_serial_flush(lua_State *L) {
    ...
    LOG("Successfully finished ltf-serial flush.");
    return 0;
}

int l_module_serial_drain(lua_State *L) {
    LOG("Invoked ltf-serial drain...");
    int s = selfshift(L);
    l_module_serial_t *u = check_port(L, s);
    ...
    LOG("Successfully finished ltf-serial drain.");
    return 0;
}

int l_module_serial_close(lua_State *L) {
    LOG("Invoked ltf-serial close...");
    int s = selfshift(L);
    l_module_serial_t *u = check_port(L, s);
    if (u->port) {
    ...
    LOG("Successfully finished ltf-serial close.");
    return 0;
}

static int l_gc(lua_State *L) {
    LOG("Invoked ltf-serial port GC...");
...
int l_module_serial_register_module(lua_State *L) {
    LOG("Registering ltf-serial module...");

    luaL_newmetatable(L, "ltf-serial");
    ...
    LOG("Successfully registered ltf-serial module.");
    return 1;
}
