...
void ltf_tui_hook_log(ltf_state_test_output_t *output) {
...
static void ltf_tui_project_header_render(pico_t *ui) {
    ...
    pico_ui_puts_yx(ui, 0, 4, "v" LTF_VERSION);
    ...
    pico_ui_printf_yx(ui, 3, 17, "%s", ltf_state->project_name);
    ...
    pico_ui_printf_yx(ui, 4, 17, "%s",
                      ltf_state->target ? ltf_state->target : "none");
    ...
    for (size_t i = 0; i < vars_count; ++i) {
        ltf_var_entry_t *e = da_get(ltf_state->vars, i);
        pico_set_colors(ui, PICO_COLOR_BRIGHT_YELLOW, -1);
    ...
    pico_ui_printf_yx(ui, 7, 17, "%s",
                      ltf_log_level_to_str(ui_state.log_level));
    pico_reset_colors(ui);
}

static void ltf_tui_test_progress_render(pico_t *ui) {
...
static void ltf_tui_summary_render(pico_t *ui, size_t size) {
...
static int test_result_to_color(ltf_state_test_t *test) {
...
void ltf_tui_defer_queue_started(ltf_state_test_t *) { render_ui(ui, NULL); }

void ltf_tui_defer_queue_finished(ltf_state_test_t *) { render_ui(ui, NULL); }
...
void ltf_tui_hook_finished(ltf_hook_fn) { render_ui(ui, NULL); }
...
int ltf_tui_init(ltf_state_t *state) {
    ...
    // ltf_state_register_test_run_started_cb();
    ltf_state_register_test_started_cb(state, ltf_tui_test_started);
    ltf_state_register_test_finished_cb(state, ltf_tui_test_finished);
    ltf_state_register_test_log_cb(state, ltf_tui_log);
    ltf_state_register_test_teardown_started_cb(state,
                                                ltf_tui_defer_queue_started);
    ltf_state_register_test_teardown_finished_cb(state,
                                                 ltf_tui_defer_queue_finished);
    ltf_state_register_test_defer_failed_cb(state, ltf_tui_defer_failed);
    ltf_state_register_test_run_finished_cb(state, ltf_tui_tests_set_finished);

    ltf_hooks_register_hook_started_cb(ltf_tui_hook_started);
