...
ltf.test({
	...
	body = function()
		...
		local function test_logging_tests(log_test, log_level, amount, status, additional)
			...
			util.test_tags(log_test, { "module-ltf", "logging" })
		...
		util.test_tags(test, { "module-ltf", "logging" })
...
ltf.test({
	...
	body = function()
		...
		util.test_tags(test, { "module-ltf", "utils" })
		...
		util.test_tags(test, { "module-ltf", "utils", "some-other-tag" })
		util.error_if(#test.output ~= 2, test, "Outputs not match")
		check.check_output(test, test.output[1], "utils", "INFO")
